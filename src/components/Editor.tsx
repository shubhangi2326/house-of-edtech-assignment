...
export default function Editor({ docId, role }: { docId: string, role: string }) {
  ...
  // 2. EDITOR
  const editor = useEditor({
    extensions: [StarterKit.configure({ history: false } as any), Collaboration.configure({ document: ydoc }), ImageExtension.configure({ inline: true, allowBase64: true })],
    editable: role !== 'VIEWER', immediatelyRender: false,
  });
  ...
  const sendMessage = async () => {
    ...
    const userMsg = { role: "user", content: input || "Image Sent", image: pendingImage };
  ...
  const startListening = () => {
    const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (!SR) return alert("No voice support.");
    const rec = new SR();
    rec.onstart = () => setIsListening(true);
  ...
  if (isInitialSyncing) return <div className="h-screen w-full flex items-center justify-center font-black uppercase text-slate-400">Restoring Workspace...</div>;

  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden relative font-sans">
      
      {/* SIDEBAR - Passes close logic */}
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        ...
        <footer className="h-12 bg-white border-t flex items-center justify-between px-8 text-[10px] font-black uppercase shrink-0">
           <div className="flex gap-4"><a href="https://github.com/shubhangi2326" target="_blank" className="text-indigo-600">GITHUB</a><a href="https://linkedin.com/in/shubhangi-mahajan2" target="_blank" className="text-indigo-600">LINKEDIN</a></div>
